...
public:
	...
	void DrawLineBresenham(float x1, float y1, float x2, float y2, Color c);
	void DrawLineBresenhamBot(float x0, float y0, float x1, float y1, Color c);
	void DrawLineBresenhamTop(float x0, float y0, float x1, float y1, Color c);
	void DrawLineBresenham(const Vec2& p0, const Vec2& p1, Color c)
	{
		if (abs(p1.y - p0.y) < abs(p1.x - p0.x))
			if (p0.x > p1.x)
				DrawLineBresenhamBot(p1.x, p1.y, p0.x, p0.y, c);
			else
				DrawLineBresenhamBot(p0.x, p0.y, p1.x, p1.y, c);
		else
			if (p0.y > p1.y)
				DrawLineBresenhamTop(p1.x, p1.y, p0.x, p0.y, c);
			else
				DrawLineBresenhamTop(p0.x, p0.y, p1.x, p1.y, c);
	}
